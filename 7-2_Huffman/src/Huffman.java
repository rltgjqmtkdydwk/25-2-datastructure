public class Huffman {
	...
	private int N; // 힙의 크기
	private int totalBits = 0; // 허프만 압축 후 전체 비트 수
	...
	public void preorder(Entry node) { // 전위순회하며 허프만 코드를 만들고, 각 이파리 노드에서 코드 출력
		...
		if (node.getValue().length() == 1) { // 현재 노드가 이파리 노드이면, 허프만 코드 출력
			...
			int bits = node.getCode().trim().length() * node.getKey(); // 문자의 빈도 × 코드 길이
			totalBits += bits;
		}
	}
	...
	public Entry deleteMin() {
		Entry min = a[1]; // a[1]의 최소값을 min으로
	...
	private void downheap(int i) { // i는 현재 노드의 인덱스
		while (2 * i <= N) { // i의 왼쪽 자식노드가 힙에 있으면
			...
			if (k < N && greater(k, k + 1))
				k++; // 왼쪽과 오른쪽자식의 승자를 결정하여 k가 승자의 인덱스가됨
			if (!greater(i, k))
